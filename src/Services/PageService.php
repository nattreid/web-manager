...
class PageService
{
	 ...
	 * @param string $url
	 */
	public function createRoute(RouteList $routes, string $url): void
	{
	 ...
	 * @param string $url
	 */
	public function createPageRoute(RouteList $routes, string $url): void
	{
	 ...
	 * @param string $url
	 */
	public function createDefaultPageRoutes(RouteList $routes, string $url): void
	{
		if ($this->configurator->onePage) {
			$routes[] = new Route($url, $this->onePageLink);
			$routes->addRoute($url . 'index.php', $this->onePageLink, Route::ONE_WAY);
			$routes[] = new Route($url . '<presenter>[/<action>]', $this->defaultLink);

		} else {
			$routes[] = new Route($url, $this->defaultLink);
			$routes->addRoute($url . 'index.php', $this->defaultLink, Route::ONE_WAY);
			$routes[] = new Route($url . '<presenter>[/<action>]', $this->defaultLink);
		}
	}
